...
- لایه‌های سخت‌افزاری (Hardware Layers) و سامانه‌های توزیع‌شده (Distributed Systems) موردنیاز برای آموزش LLM را درک کنید.
- گلوگاه‌های حافظه، محاسبات، ارتباطات و خط لوله‌ی داده (Data Pipeline) را تحلیل کنید.
- موازی‌سازی داده، تنسور، خط لوله، توالی/زمینه، متخصص و روش‌های ترکیبی را با یکدیگر مقایسه کنید.
- از پروفایل‌سازی، مشاهده‌پذیری، نقطه‌های بررسی، تحمل خطا و معیارهای هزینه برای اجرای قابل‌اعتماد فرایند آموزش استفاده کنید.
...
- **بهره‌برداری پایین از شتاب‌دهنده‌ها (Low Accelerator Utilization):** پیش از بهینه‌سازی مشخص کنید که گلوگاه اصلی اجرای آموزش، ورودی داده (Input)، حافظه (Memory)، محاسبات (Compute) یا ارتباطات (Communication) است.
- **غلبه‌ی سربار نقطه‌های بررسی (Checkpoint Overhead):** پهنای باند ذخیره‌سازی، زمان سریال‌سازی (Serialization) و تناوب ایجاد نقطه‌های بررسی را اندازه‌گیری کنید.
- **فروپاشی بهره‌وری مقیاس‌پذیری (Scaling Efficiency Collapse):** همگام‌سازی (Synchronization)، ارتباطات جمعی (Collective Communication)، نمونه‌های کندتر (Stragglers) و عدم تعادل بار (Load Imbalance) را بررسی کنید.

## پرسش‌های مرور (Review Questions)

1. یک فرایند آموزش چگونه می‌تواند محاسبه‌محدود (Compute-Bound)، حافظه‌محدود (Memory-Bound)، ارتباطات‌محدود (Communication-Bound) یا ورودی‌محدود (Input-Bound) باشد؟
2. در چه شرایطی شاردینگ (Sharding) نسبت به موازی‌سازی داده‌ی ساده (Simple Data Parallelism) گزینه‌ی مناسب‌تری است؟
3. پیش از تلاش برای بهینه‌سازی در سطح سامانه (Systems Optimization)، چه اندازه‌گیری‌ها و معیارهایی را باید جمع‌آوری کنید؟
